...
function handle() {
    return async ({ event, resolve }) => {
        ...
        event.locals.canI = pipe({ route: event.route.id, method: event.request.method }, canI);
        return await resolve(event);
    };
}
...
const throwOrWarn = (message, warn) => {
    if (warn)
        return pipe(message, printWarn, () => false);
...
export { CanIKitPolicy, handle as canIKitHandle, plugin as canIKitPlugin };
//# sourceMappingURL=can-i-kit.esm.js.map
