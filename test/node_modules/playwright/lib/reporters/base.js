...
class BaseReporter {
  ...
  generateSummaryMessage({
    ...
    if (expected) tokens.push(_utilsBundle.colors.green(`  ${expected} passed`) + _utilsBundle.colors.dim(` (${(0, _utilsBundle.ms)(this.result.duration)})`));
  ...
  _printMaxFailuresReached() {
    ...
    console.log(_utilsBundle.colors.yellow(`Testing stopped early after ${this.config.maxFailures} maximum allowed failures.`));
  }
      ...
      for (let i = 0; i < result.attachments.length; ++i) {
        ...
        if (attachment.path) {
          ...
          if (attachment.name === 'trace') {
            ...
            resultLines.push('');
            resultLines.push(_utilsBundle.colors.cyan(`        ${packageManagerCommand} playwright show-trace ${relativePath}`));
            resultLines.push('');
          }
        } else {
