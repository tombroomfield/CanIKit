...
function getHtmlReportOptionProcessEnv() {
  ...
  if (!isHtmlReportOption(htmlOpenEnv)) {
    console.log(_utilsBundle.colors.red(`Configuration Error: HTML reporter Invalid value for ${processKey}: ${htmlOpenEnv}. Valid values are: ${htmlReportOptions.join(', ')}`));
    return undefined;
  }
  return htmlOpenEnv;
}
...
class HtmlBuilder {
  ...
  async build(metadata, projectSuites, result) {
    const data = new Map();
