...
class JobDispatcher {
  ...
  _massSkipTestsFromRemaining(testIds, errors, onlyStartedTests) {
    for (const test of this._remainingByTestId.values()) {
      ...
      if (!this._failureTracker.hasReachedMaxFailures()) {
        ...
        if (runData) {
          ...
          if (onlyStartedTests && this._currentlyRunning) continue;
  ...
  _onDone(params) {
    ...
    if (params.fatalUnknownTestIds) {
      const titles = params.fatalUnknownTestIds.map(testId => {
        ...
        return test === null || test === void 0 ? void 0 : test.titlePath().slice(1).join(' > ');
      }).filter(title => !!title);
    ...
    if (params.unexpectedExitError) this._massSkipTestsFromRemaining(new Set(this._remainingByTestId.keys()), [params.unexpectedExitError], true /* onlyStartedTests */);
